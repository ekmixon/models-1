...
class Config(params_dict.ParamsDict):
  ...
  * Warning: it converts Dict to `Config` even within sequences,
  ...
  def _isvalidsequence(cls, v):
    ...
    """
    return ((all(isinstance(e, cls.IMMUTABLE_TYPES)
                 for e in v) or all(isinstance(e, dict) for e in v)
             or all(isinstance(e, params_dict.ParamsDict)
                    for e in v)) if isinstance(v, cls.SEQUENCE_TYPES) else False)
